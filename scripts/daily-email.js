...
const MEALS = [
  ...
  { name: '周五', lunch: '100g🍚米饭 + 🥩100g瘦猪肉（猪腱子，少油炒肉丝配菠菜） + 🥬清炒芦笋+生菜', dinner: '200g🍠紫薯 + 🥩100g瘦牛肉（牛腱子，清炖撇浮油，仅吃肉）', nut: '15g🥜原味核桃' },
  {
    ...
    lunch:
      '120g🍚生米煮饭 + 🍗2个去皮鸡腿（清炖加姜片，不喝浓汤） + 🥬香菇+油麦菜清炒',
    dinner: '250g🍠紫薯 + 🥩100g瘦牛肉（牛腱子，清炖撇浮油，仅吃肉）',
...
const COMMON = {
  breakfast:
    '60g🥣燕麦 + 2🥚白煮蛋 + 1小盒🥛无糖纯酸奶 + 半盒🫐蓝莓（加25g酵母蛋白粉，推荐提前一晚将燕麦加约150ml水拌匀后冷藏，早上直接食用）',
...
function buildPlan(offsetDays) {
  ...
  const breakfast = (meal && meal.breakfast) || COMMON.breakfast;
  const beforeBed = (meal && meal.beforeBed) || COMMON.beforeBed;
